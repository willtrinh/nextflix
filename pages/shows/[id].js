...
import { getShowDetails } from '../../helpers/api-utils';
import { CalendarIcon, ClockIcon, XIcon } from '@heroicons/react/outline';
import ReactPlayer from 'react-player/lazy';

const ShowDetailsPage = ({ result }) => {
      ...
      <section className='relative z-50'>
        ...
        <div className='absolute inset-y-28 md:inset-y-auto md:bottom-10 inset-x-4 md:inset-x-12 space-y-6 z-50'>
          <h1 className='text-3xl sm:text-4xl md:text-5xl font-bold'>
            {result.name || result.original_name} {`(${releaseYear})`}
          </h1>
          <div className='flex items-center space-x-3 md:space-x-5'>
            <button
              className='font-bold text-xs md:text-base bg-black/60 text-[#f9f9f9] border border-[#f9f9f9] flex items-center justify-center py-2.5 px-6 rounded hover:bg-slate-50 hover:text-black transition duration-200 transform hover:scale-110'
